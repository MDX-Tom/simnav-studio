...
final class PlannerService: @unchecked Sendable {
    ...
    private func emptyRoutePayload(message: String) -> [String: Any] {
        [
            "departure": NSNull(),
            "arrival": NSNull(),
            "legs": [],
            "points": [],
            "segments": ["departure": [], "enroute": [], "arrival": []],
            "generated": false,
            "route_display": "",
            "distance_nm": 0,
            "selected_procedures": [:],
            "selected_runways": ["departure": "ALL", "arrival": "ALL"],
            "message": message
        ]
    }
    ...
    private func pointRows(
        ...
        let bounds = pointBoundsSQL(latColumn: latColumn, lonColumn: lonColumn, south: south, north: north, west: west, east: east)
        return try database.rows(
            sql: "\(sqlPrefix) where \(latColumn) is not null and \(lonColumn) is not null and \(bounds.sql) \(orderLimit)",
            arguments: bounds.arguments
        )
    }

    private func navaidRows(database: SQLiteDatabase, south: Double, west: Double, north: Double, east: Double, limit: Int) throws -> [[String: Any]] {
        let vorBounds = pointBoundsSQL(latColumn: "vor_latitude", lonColumn: "vor_longitude", south: south, north: north, west: west, east: east)
        let ndbBounds = pointBoundsSQL(latColumn: "ndb_latitude", lonColumn: "ndb_longitude", south: south, north: north, west: west, east: east)

        let vors = try database.rows(
            sql: """
            select vor_identifier as ident,
                   area_code,
                   coalesce(vor_name, vor_identifier) as name,
                   ...
                   'vor' as kind
            from tbl_vhfnavaids
            where vor_latitude is not null and vor_longitude is not null and \(vorBounds.sql)
            order by vor_identifier
            limit ?
            """,
            arguments: vorBounds.arguments + [.int(limit)]
        )

        let ndbs = try database.rows(
            sql: """
            select ndb_identifier as ident,
                   area_code,
                   coalesce(ndb_name, ndb_identifier) as name,
                   ...
                   'ndb' as kind
            from tbl_enroute_ndbnavaids
            where ndb_latitude is not null and ndb_longitude is not null and \(ndbBounds.sql)
            order by ndb_identifier
            limit ?
            """,
            arguments: ndbBounds.arguments + [.int(limit)]
        )

        return Array((vors + ndbs).prefix(limit))
    }

    private func airwaySegments(database: SQLiteDatabase, south: Double, west: Double, north: Double, east: Double, limit: Int) throws -> [[String: Any]] {
        let bounds = pointBoundsSQL(latColumn: "waypoint_latitude", lonColumn: "waypoint_longitude", south: south, north: north, west: west, east: east)
        let rows = try database.rows(
            sql: """
            select route_identifier, seqno, waypoint_identifier, waypoint_latitude, waypoint_longitude,
                   direction_restriction, outbound_course, inbound_course, area_code, icao_code, route_type
            from tbl_enroute_airways
            where waypoint_latitude is not null and waypoint_longitude is not null and \(bounds.sql)
            order by route_identifier, cast(seqno as integer), area_code, icao_code, route_type
            limit ?
            """,
            arguments: bounds.arguments + [.int(limit + 200)]
        )
        ...
        for row in rows {
            ...
            let previousSeq = navInt(previous["seqno"]) ?? 0
            let currentSeq = navInt(row["seqno"]) ?? 0
            guard currentSeq > previousSeq, abs(currentSeq - previousSeq) <= 5000,
                  let previousLat = navDouble(previous["waypoint_latitude"]),
                  let previousLon = navDouble(previous["waypoint_longitude"]),
                  let currentLat = navDouble(row["waypoint_latitude"]),
                  let currentLon = navDouble(row["waypoint_longitude"]) else {
            ...
            let path = [[previousLat, previousLon], [currentLat, displayLon]]
            output.append(segmentRecord(
                name: navString(row["route_identifier"]),
                entry: navString(previous["waypoint_identifier"]),
                exit: navString(row["waypoint_identifier"]),
                path: path,
    ...
    private func runwaySegments(database: SQLiteDatabase, south: Double, west: Double, north: Double, east: Double) throws -> [[String: Any]] {
        let bounds = pointBoundsSQL(latColumn: "runway_latitude", lonColumn: "runway_longitude", south: south, north: north, west: west, east: east)
        let rows = try database.rows(
            sql: """
            select airport_identifier as airport,
                   runway_identifier as ident,
    ...
    private func pointBoundsSQL(
        ...
        west: Double,
        east: Double
    ) -> (sql: String, arguments: [SQLiteArgument]) {
        let ranges = longitudeRanges(west: west, east: east)
        var clauses: [String] = []
        var arguments: [SQLiteArgument] = [.double(south), .double(north)]
        for range in ranges {
            clauses.append("(\(lonColumn) between ? and ?)")
            arguments.append(.double(range.lowerBound))
            arguments.append(.double(range.upperBound))
        }
        let lonSQL = clauses.isEmpty ? "1 = 1" : clauses.joined(separator: " or ")
        return ("(\(latColumn) between ? and ?) and (\(lonSQL))", arguments)
    }

    private func longitudeRanges(west: Double, east: Double) -> [ClosedRange<Double>] {
        let firstCopy = Int(floor((west + 180) / 360)) - 1
        let lastCopy = Int(floor((east + 180) / 360)) + 1
        var ranges: [ClosedRange<Double>] = []
        for copy in firstCopy...lastCopy {
            let offset = Double(copy) * 360
            let copyWest = -180 + offset
            let copyEast = 180 + offset
            let overlapWest = max(west, copyWest)
            let overlapEast = min(east, copyEast)
            if overlapWest <= overlapEast {
                ranges.append((overlapWest - offset)...(overlapEast - offset))
            }
        }
        return ranges
    }
