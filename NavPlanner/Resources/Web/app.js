...
const state = {
  ...
  calculateManufacturer: CALC_DEFAULT_MANUFACTURER,
  calculateAircraft: CALC_DEFAULT_AIRCRAFT,
...
function syncCalculateAircraftControls({ resetMach = false } = {}) {
  ...
  if (elements.calcCruiseMachTicks) {
    ...
    elements.calcCruiseMachTicks.innerHTML = [
      { value: 0.5, label: "M0.50" },
      { value: aircraft.econMach, label: `${formatMach(aircraft.econMach)} ECON` },
      { value: max, label: `${formatMach(max)} MAX` },
    ].map((tick) => {
      const position = ((tick.value - 0.5) / Math.max(0.01, max - 0.5)) * 100;
      return `<span style="--tick-position: ${position.toFixed(3)}%;"><i></i><b>${escapeHtml(tick.label)}</b></span>`;
    }).join("");
  }
}
...
async function applySimulatorDebugLaunch() {
  ...
  if (CALC_WEATHER_SOURCE_KEYS.has(config.weatherSource)) {
