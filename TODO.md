...
- [x] 移除 Plan 中旧的“匹配轨迹”按钮，保留 `生成并绘制航路 / 重新计算` 两个规划按钮，并在计划页提供“清除轨迹绘制”快捷按钮。
- [x] 将 iPhone 底部三标签再贴近底部，输入区比例收紧到约 36%，释放更多地图高度。
- [x] 为 iPhone 底部三标签恢复 Home Indicator 安全区留白，避免标签贴到系统横条。
- [x] 微调 iPhone 底部四标签位置：整体向下 14px，左右各内收 20px，并将标签预留行收窄到 9px 以同步增加中部工作区高度。
- [x] 为 iPhone 下部工作区增加上拉手柄：默认保持当前约 66% 地图 / 34% 工作区，最大上拉时地图保留 30%；切换 `计划 / 机场 / 查询 / 设置` 不重置位置，软键盘出现时继续走原有自动上拉布局。
- [x] 优化 iPhone 上拉手柄触控：banner 高度和面板顶部预留均收敛为 20px，外层光晕围绕短条等距外扩，轻点可在最低位 / 最高位之间切换，拖动仍用 `requestAnimationFrame` 节流并按 0.2% 量化面板高度更新，减少上划掉帧。
- [x] 二次优化 iPhone 上拉手柄拖动帧率：拖动中不再反复写入面板状态 dataset，临时关闭面板毛玻璃 / 重阴影，启用 layout/paint containment，并把 MapLibre `ResizeObserver` 同步、Leaflet resize 和矢量底图 resize 延后到释放后统一执行。
- [x] 三次优化 iPhone 上拉手柄拖动帧率：回滚改变视觉效果的 sheet 预览方案，恢复拖动时同步调整地图 panel 和输入 panel 尺寸；拖动比例按地图行 + 面板行的真实轨道高度计算，每帧改为写入固定像素行高变量，减少 `fr` 行高分配计算，释放后再提交持久布局和地图内部 resize。
- [x] 修复 iPhone 上拉手柄按住但不拖动时 panel 尺寸异常：按压态与拖动态分离，未超过 6px 阈值只播放手柄 transform 按压动画，不进入最大高度 sheet 预览。
- [x] 修复 iPhone 上拉手柄轻点穿透：轻点展开 / 收起后短暂拦截 WKWebView 合成 click / focus，避免布局变化后额外点中地图弹窗或“手动机场”输入框。
- [x] 将 iPhone 主航路、SID / STAR / APPROACH 和 FR24 轨迹可见线宽降为 iPad 的 80%，点击命中层保持原宽。
- [x] 略增 iPhone 地图 `+ / -` 缩放按钮之间的间距。
- [x] 修复 iPhone 地图 `+ / -` 缩放按钮圆角覆盖问题，让每个按钮四角独立圆角。
- [x] 在地图左上角增加叠加层开关，可分别显示 / 隐藏地图层、蓝色自动 / 匹配航路与 nav-overlay 蓝色 airway、黄色人工航路、SID / STAR / APPROACH、FR24 轨迹、terminal waypoints 和其他航点 / 导航台；隐藏态只置灰图标，按钮本体背景、阴影和底纹保持不变。
...
- [x] 补强 FR24 历史展示：历史页优先按 DOM 表头 / 单元格解析时间、机型、状态和起降机场；实际起飞 / 落地时间按页面展示时区解析，`Scheduled` 状态只保留 1 条，历史记录以每行 2 个卡片展示；若实际 From / To 与预设航线不一致，会标注实际起降机场；Query 页支持手动输入航班号或 flightId 查询历史 / 生成可下载记录。
- [x] 补强 FR24 轨迹绘制：相邻轨迹点距离超过 10nm，或较长稀疏采样段两端存在明显航向突变时，该段使用黑色虚线，正常轨迹段保持黑色实线。
- [x] 补强规划航路绘制：按航点绘制的主航路和 SID / STAR / APPROACH 会突出显示全部参与绘制的航点，复用 Procedure 表格行点击的脉冲高亮动画。
- [x] 增加 FR24 下载缓存检索与管理：Query 页可搜索本机已下载轨迹，按历史航班卡片展示基本信息，支持绘制本地轨迹、单项删除、收藏 / 取消收藏；一键删除下载缓存会跳过收藏文件。
