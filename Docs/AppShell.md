...
- 详情区域加入 `Airport / Settings` 切换，不改变计划栏、地图区和原有 Airport 面板结构。
...
- 当前 iPhone 竖屏加载 Web 参考版移动布局：上部固定为地图，最新比例约占屏幕 66%；下部为输入工作区，约占屏幕 34%，底部三标签另占一条约 19px 的低高度控制行。
- 下部工作区提供 `计划`、`机场`、`设置` 三个中文图标标签，切换标签只改变下部内容，不销毁或重建顶部地图实例。
...
- iPhone 横屏使用 iPad 横屏式工作台：左侧计划栏、中间地图、右侧详情/设置栏和左右折叠按钮同时显示；底部三标签隐藏，右侧继续使用 `机场 / 设置` 标签切换。紧凑工作台媒体查询已从 920px 扩展到 1024px，iPhone 17 Pro Max 等更宽横屏机型也会使用和 iPhone 17 Pro 一致的 UI。
...
- iPhone 切换底部 `计划 / 机场 / 设置` 到不同标签时会主动取消输入框焦点；重复点击当前标签不再 blur，避免正在编辑的输入框被误收起。
- `MapWebView` 已接入 `WKUIDelegate` 的 alert / confirm / prompt 面板。Web 版 `Match FR24` 在线失败后使用 `window.prompt` 粘贴轨迹点，iOS 现在会弹出原生输入框并把内容回传给 Web 工作台。
