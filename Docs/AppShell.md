...
- 下部工作区顶部新增上拉手柄，视觉参考系统输入面板的短灰色拖拽条；用户可把工作区上拉，地图区域最小保留 30%，下拉回默认 66% / 34%。手柄状态保存在当前 Web 工作台内，切换 `计划 / 机场 / 查询 / 设置` 不重置高度；软键盘出现时继续由 `visualViewport` 键盘态布局接管，手柄隐藏且不影响自动上拉。
