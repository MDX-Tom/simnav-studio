...
- 下部工作区面板内部顶部新增上拉手柄，视觉参考系统输入面板的短灰色拖拽条；用户可把工作区上拉，地图区域最小保留 30%，下拉回默认 66% / 34%。轻点手柄会在最低位和最高位之间切换，拖动仍可连续调整；手柄状态保存在当前 Web 工作台内，切换 `计划 / 机场 / 查询 / 设置` 不重置高度；软键盘出现时继续由 `visualViewport` 键盘态布局接管，手柄隐藏且不影响自动上拉。当前手柄高度和面板顶部预留均为 20px，外层胶囊光晕围绕短条等距外扩。为保持视觉效果不变，拖动时仍同步调整地图 panel 和输入 panel 的尺寸；优化只发生在实现层：超过 6px 阈值后才进入拖动态，按地图行 + 面板行的真实轨道高度计算指尖位移，并每帧写入固定像素行高 `--mobile-map-row-size` / `--mobile-panel-row-size`，避免实时使用 `fr` 行高参与分配；释放时再提交持久 `--mobile-map-flex` / `--mobile-panel-flex`。MapLibre `ResizeObserver` 同步、Leaflet `invalidateSize` 和矢量底图 resize 仍延后到释放后统一执行，但外层 map/panel 尺寸在拖动中同步变化。拖动态还会临时关闭面板毛玻璃 / 重阴影并启用 layout/paint containment，降低小屏拖动时的主线程重排和重绘压力；手柄按压动画改为 transform 缩放，避免改变宽高触发布局。轻点手柄后会短暂启用捕获期点击护栏，吞掉 iOS / WKWebView 在 `pointerup` 后补发的兼容 click / focus 事件，避免面板完成展开或收起后额外点中下方地图弹窗或“手动机场”输入框。
- 底部标签栏在 iPhone 网格中独占第三行，使用更低高度的紧凑玻璃质感，并通过 `env(safe-area-inset-bottom)` 为 iOS Home Indicator 保留距离；当前标签栏整体向下 14px，左右各内收 20px，让标签不再贴满下部面板边缘；标签预留行收窄到 9px，把高度同步还给中部工作区；内容面板高度截止到标签栏上方，不再通过覆盖式悬浮占用表单空间。
...
- iPhone 竖屏 / 横屏的主航路、SID / STAR / APPROACH 和 FR24 轨迹可见线宽为 iPad 的 80%，但点击命中层保持原宽，避免小屏视觉过重同时不牺牲触控。
- 计划页主按钮文字已调整为“生成并绘制航路”，`重新计算` 右侧提供“清除轨迹绘制”；查询页把“清除轨迹绘制”和“还原轨迹匹配”移动到“查询”按钮右侧，缓存卡片底部只保留缓存删除和刷新状态。
- 地图左上角使用与缩放 / 地图类型控件一致的纵向按钮组管理叠加层显隐，隐藏态只置灰图标，不改变按钮背景、阴影和底纹；第二个“航路”按钮会同时控制蓝色自动 / 匹配航路以及 nav-overlay 蓝色 airway。标签切换和 iPhone 上拉手柄不销毁地图实例，因此叠加层显隐状态会随当前 Web 工作台保持。
