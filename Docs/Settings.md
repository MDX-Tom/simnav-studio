...
- iPhone：Settings 是下部工作区的第四个标签，切换时顶部地图保持原实例、视角和叠加层。
- iPhone：底部入口显示为 `设置` 中文图标标签，标签栏使用紧凑玻璃质感并独占底部行，Settings 内容区截止在标签栏上方。
- iPad：Settings 位于详情区域的 `Airport / Query / Settings` 切换中，计划栏和地图区保持既有工作台状态。
- 数据库：通过 `window.webkit.messageHandlers.navplanner` 发送 `selectDatabase`，Swift 侧弹出系统文件选择器。
- 支持文件扩展名：`.s3db`、`.sqlite`、`.sqlite3`、`.db`。
- 导入路径：选中文件复制到 `Application Support/NavPlanner/Database/`，文件名清洗后统一使用 `.sqlite`；若同名文件已存在，会保留原文件并使用递增后缀，避免覆盖用户已有数据库。
- 导入成功后：Swift 重新打开 SQLite 连接，Web 侧刷新 header、数据库状态、本地数据库列表和 nav-overlay，并清理与数据库相关的前端缓存。
- 本地数据库存储管理：Settings 中的数据库卡片位于应用图标与离线地图之间，显示数据库文件数量、空间占用、当前文件、AIRAC / 修订、大小和修改时间；支持刷新 / 搜索、切换可读数据库、删除非当前非内置数据库、恢复并启用内置 `navdata.sqlite`。这些操作通过 Swift 本地 `/api/databases/list`、`select`、`delete`、`restore-bundled` 完成，不依赖远程服务。
- 数据库状态：Settings 卡片使用中文描述 AIRAC 与修订号；Plan 启动时不再显示“本地数据库已载入...”状态行，只保留航路输入 / 规划相关状态。
...
- FR24 会话和下载缓存不放在 Settings 中管理，而是在 `查询` Tab 内完成：访问卡片可打开 App 内 FR24 验证页并自动同步浏览器会话，状态只显示“浏览器会话已同步 / 未同步”；手动 Cookie / `_frPl` 输入仅作为高级可选兜底。底部单独显示 FR24 缓存文件数和大小，删除时只清理 Caches 中的 FR24 GPX / playback JSON / meta 文件，不影响在线地图缓存、离线地图包和导航数据库。
...
- 版权与说明：页面先介绍 NavPlanner 的本地航路规划、机场 / Procedure 查看、FR24 轨迹查询绘制和 Settings 管理方式，最后说明地图底图、离线地图包和导航数据版权归各自数据来源所有，App 开发者为 MDX；地图右下角不再显示 attribution 水印。
- Xcode 运行期日志策略：不会用 `OS_ACTIVITY_MODE=disable` 等方式整体屏蔽控制台。已修复 App 相关的 bundle sandbox extension、WebP 探测和 iOS 27 beta PointerUI / `statusBarOrientation` 红字；剩余 `cfprefsd Couldn't open ... plist` 属于模拟器系统偏好探测噪声，保留日志可见性以便继续捕获真正的 WebBridge、FR24、离线地图和本地服务错误。

## 触控约束

- Settings 内按钮、卡片、标题和说明文字使用更小尺寸，适配 iPhone 下部约 34% 工作区。
- iPhone 底部标签固定在独立底部行，Settings 内容通过内部滚动查看，不被标签栏遮挡。
- iPhone 底部标签二次压缩高度，贴近底部安全区，同时保持 `计划 / 机场 / 查询 / 设置` 图标和文字可读。
...
- 主地图允许 Leaflet 双击放大；页面空白处仍安装 document 级双击 / 双触防放大保护，避开输入框、按钮、链接、Leaflet 控件和离线地图弹窗。
- iPhone 输入框唤起软键盘时，Web 工作台缩到键盘上方可见高度并临时隐藏底部 Tab，当前输入面板停在键盘上方；点击系统对勾或键盘收起后恢复原布局。

## 后续

- 增加导入前 schema 版本和兼容性细分检查，并在数据库列表中给出更明确的缺表 / 字段缺失提示。
