...
- FR24 在线增强已从 Plan 旧按钮迁移到独立 `查询` Tab：Swift 本地 `/api/fr24/search` / `history` / `manual-history` / `download` / `cache` / `access` 负责 FR24 Web 会话状态、App 内 FR24 验证浏览器 CookieStore 自动同步、航线航班查询、按 `www.flightradar24.com/data/flights/{flight}` 数据页读取历史子菜单、GPX / playback JSON 缓存、缓存检索、单项删除和收藏保护；历史页优先按 DOM 表头 / 单元格解析机型、计划 / 实际起降时间和状态，并只保留 1 条 `Scheduled` 记录；前端可将下载轨迹或本地缓存轨迹用黑色线绘制到地图，超过 10nm 或疑似稀疏圆弧拉直的轨迹段显示为黑色虚线，并继续 POST `/api/route/track-match` 执行本地匹配。`/api/route/fr24-match` 保留 Web parity 入口并改为在线尝试。
...
- 完整航路解析和自动航路规划。
- FR24 Web 真实公网可用性和错误细节：App 内同步 Cookie / `_frPl`、Cloudflare 验证、401 / 403、轨迹不足、无历史航班、飞行中航班、会话配置状态，以及固定 `ZULS` / `ZUAL` 航线的 FR24 回归验证。
