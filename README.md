...
| 📡 | **FR24 track comparison** | Query first through the shared backend. Only if FR24 requests verification, open the App-owned WebKit session on Apple/macOS Local Web or a private Chromium session on Windows/Linux Local Web; Local Web syncs and closes it automatically. Download, draw, and match tracks without an official API credential; GPX and FR24 CSV/KML import remain available. |
...
3. Only if FR24 actually requests verification, explicitly open **FR24 Verification** and complete the normal FR24 / Cloudflare check. Apple and macOS Local Web use an App-owned WebKit session, so they do not launch Edge, Chrome, or the user's default browser. Windows/Linux Local Web use a private Chrome/Chromium/Edge session selected from the actual environment. Local Web transfers the verified session and closes the page automatically. Schedule, history, and playback continue through the shared Swift backend rather than browser data tabs.
...
> **Online enhancement.** FR24 is optional. Local Web keeps its private verification session and copied FR24 state under the independent Local Web data root and never reads the user's normal browser profile. Startup, schedule lookup, history, and playback use the shared Swift FR24 backend without opening browser data pages. A verification window is shown only after the user explicitly chooses **Open FR24 Verification Page**, and it closes after the session is accepted. macOS owns that window through WebKit; Windows/Linux Chromium fallback keeps its randomized DevTools listener on private loopback only. Network, session, or verification failures never block local planning, airport lookup, procedures, nav overlays, or offline maps. SimNav Studio does not bypass Cloudflare or automate CAPTCHA challenges.
...
and runs the native Swift executable inside the pinned container. macOS Local Web owns its explicit
FR24 verification window through WebKit and never launches a third-party browser. Windows/Linux use
an isolated Chrome/Chromium/Edge fallback selected from the actual environment only for that explicit
verification action; Linux connects through a private Compose-gateway relay, while Windows uses an
ephemeral authenticated host relay. Normal Query, history, and playback requests use the shared Swift
backend without a browser data page or a second FR24 backend. Chromium fallback keeps its randomized
DevTools port on private loopback; only the private relay can reach it in Docker mode. The v0.1.2
...
- Verify Apple/macOS Local Web App-owned WebKit and Windows/Linux private-browser FR24 sessions, Cloudflare handling, route query, playback download, GPX/CSV/KML import, profile, failed-download, drawing, matching, sharing, and cache paths.
