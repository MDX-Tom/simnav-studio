...
- **计算工作台**：选择飞机公司、具体机型、ZFW、携带燃油、巡航高度、巡航 Mach、下降率、气象数据源、重量单位和 QNH 单位，查看航路风速 / 地形剖面、地速 / 垂直速度剖面，并以 SimBrief 风格估算燃油。
...
2. 打开 **计算** 页。
3. 选择飞机公司和具体机型，调整 ZFW、携带燃油、巡航高度、巡航 Mach、下降率、气象数据源、重量单位和 QNH 显示单位。
4. 查看 SimBrief 风格的航路剖面，其中包含相对航向风、云、雨、地形、Procedure 高度限制和 QNH；通过缩放和平移 slider 调整剖面窗口，并检查地速 / 垂直速度剖面。
5. 查看 SimBrief 风格燃油估算。

当前计算模型本地优先、可离线运行。在线天气仅作为增强，不可用时会降级为本地估算；Terrarium DEM 瓦片可用时会用于地形采样，不可用时会降级为保守本地地形估算。直接气象数据授权、离线 DEM 数据包和更完整机型性能库仍是后续增强。
...
5. 对航班或历史记录选择下载并绘制轨迹，也可以手动导入 GPX 文件、查看高度 / 速度剖面，或匹配轨迹到本地航路；若加载航班的实际起降机场与“计划”页不同，匹配前会自动把实际机场同步回“计划”页。对终端采样充分、跑道判断可靠的轨迹，会先匹配完整 SID / STAR / Approach，再以 Procedure 边界为端点匹配中间航路。

下载的 FR24 轨迹会以 GPX、playback JSON 和 metadata 缓存在本机。查询页可检索缓存、绘制或匹配缓存轨迹、分享 GPX、收藏重要轨迹、打开缓存目录，并清理未收藏的下载缓存。
