...
- **计算工作台**：选择飞机公司、具体机型、巡航高度、巡航 Mach、下降率和气象数据源，查看航路风速 / 地形剖面、地速 / VS 剖面，并以 SimBrief 风格估算燃油。
...
2. 打开 **计算** 页。
3. 选择飞机公司和具体机型，调整巡航高度、巡航 Mach、下降率和气象数据源。
4. 查看航路风速 / 地面海拔剖面，必要时拖动调整航段计划高度，并检查地速 / VS 剖面。
5. 查看 SimBrief 风格燃油估算。

当前计算模型本地优先、可离线运行。真实 NOAA / ECMWF / GFS 气象格点、DEM 地形采样和更完整机型性能库是后续增强；不可用时仍保留本地估算降级。
